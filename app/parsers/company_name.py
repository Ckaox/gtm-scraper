...
def extract_company_name_from_html(html: str, domain: str, fallback_name: Optional[str] = None) -> Optional[str]:
    ...
    """
    if not html:
        return _clean_domain_name(domain) or fallback_name
...
def _clean_title_for_company_name(title: str) -> Optional[str]:
    ...
    for part in parts:
        ...
        website_words = ["home", "inicio", "welcome", "bienvenido", "página", "page", "sitio", "site"]
        if not any(word in part.lower() for word in website_words):
            candidates.append(part)
    
    if candidates:
        # Return the shortest non-generic candidate (usually the company name)
